            ...
            Some(data) => {
                ...
                let msg = format!(
                    "bisect session already in progress!\n{}\n\
                     Use '{} bisect reset' to reset your working copy",
                     stat,
                     cmd_name
                );
            ...
            None => {
                ...
                match (&good, &bad) {
                    (Some(g), Some(b)) if to_rev_num(g) == to_rev_num(b) => {
                        return Err(General(
                            "The 'good' and 'bad' revisions cannot be the same".to_string()
                        )
                        .into());
                    }
                    (Some(g), Some(b)) if to_rev_num(g) > to_rev_num(b) => {
                        return Err(General(
                            "The 'good' revision must be an ancestor of the 'bad' revision".to_string()
                        ).into());
                    }
                    _ => ()
                }
                ...
                if self.term_good.is_some() && self.term_good == self.term_bad {
                    return Err(General(
                        "The 'good' and 'bad' terms cannot be the same.".to_string()
                    ).into());
                }
                ...
                append_to_log(format!(
                    "# {} bisect log file {}",
                    cmd_name,
                    display_svn_datetime(&Local::now())
                ))?;
                append_to_log(format!(
                    "# Initiated from: {}",
