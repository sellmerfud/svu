...
impl Branch {
    ...
    fn show_list(&self, creds: &Option<Credentials>) -> Result<()> {
        ...
        if self.list_branches() {
            ...
            self.list_entries(creds, "Branches", &base_url, &sorted_prefixes, &self.branch_regexes, &all_prefixes)?
        }
        if self.list_tags() {
            let mut sorted_prefixes = prefixes.tag_prefixes.clone();
            sorted_prefixes.sort();
            self.list_entries(creds, "Tags", &base_url, &sorted_prefixes, &self.tag_regexes, &all_prefixes)?
        }
        Ok(())    
    }
    
    fn list_entries<S, T>(&self, creds: &Option<Credentials>, header: &str, base_url: &str, prefixes: &[S], regexes: &[Regex], all_prefixes: &[T]) -> Result<()> 
