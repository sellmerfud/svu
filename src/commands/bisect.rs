...
/// known to be before the bug was introduced. Then svu bisect picks a commit between those two endpoints and
/// asks you whether the selected commit is "good" or "bad". It continues narrowing down the range until it
/// finds the exact commit that introduced the change.
///
/// In fact, svu bisect can be used to find the commit that changed any property of your project; e.g., the
/// commit that fixed a bug, or the commit that caused a benchmark’s performance to improve. To support this
/// more general usage, you can redefine the terms `good` and `bad`. Type `svu bisect terms --help` for
/// more information.
#[derive(Debug, Parser)]
#[command(
    author,
